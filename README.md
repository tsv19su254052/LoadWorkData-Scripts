...
Теперь можно не писать **XSD-схему** вручную с нуля, а сгенерировать ее внутри `Management Studio` (аскетичнее, архаичнее, но срабатывает точнее)
...
или внутри `pyCharm` (удобнее в плане подсветки синтаксиса и автодополнения, есть навигация по структуре).
