...
Файлы данных `*.mdf` размещаем на корпоративном быстром желательно SAS-овском HDD средней емкости (интенсивное чтение и запись в разные места), файлы журналов `*.ldf` - на обычном SATA-овом HDD большой емкости (медленная дозапись в конец), файлы данных и журнала `tempdb` - на HDD или NVMe средней емкости.
